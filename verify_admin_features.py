...
def test_admin_features():
    ...
    response = requests.post(f"{BASE_URL}/auth/change-password", json=change_payload, headers=reg_headers)
    if response.status_code != 200:
    ...
    response = requests.post(f"{BASE_URL}/auth/token", data=admin_payload)
    if response.status_code != 200:
